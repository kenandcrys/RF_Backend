exports.up = function(knex) {
    return knex.schema.createTable('Cattle', function(table) {
      ...
      table.string('name', 255).notNullable(); // Name of the cattle feed
      ...
      table.integer('category_id').unsigned().references('id').inTable('categories').onDelete('CASCADE'); // Foreign key to the categories table
    });
  };
  
  exports.down = function(knex) {
    return knex.schema.dropTableIfExists('Cattle'); // Drop table if it exists (used for rollback)
  };
